...
class ProgressManager with ChangeNotifier {
  ...
  void _parseAndApplyData(Map<String, dynamic> data) {
    ...
    final lastActiveVal = data['lastActiveDate'];
  ...
  Future<void> _saveToLocalCache(String uid, Map<String, dynamic> data) async {
    ...
    // Sanitize data (Convert Timestamps to Strings) before saving
    final cleanData = _sanitizeForJson(data);

    final key = _getUserStorageKey(uid);
    await _prefs!.setString(key, jsonEncode(cleanData));
  }

  // --- Helper Method to prevent crash ---
  Map<String, dynamic> _sanitizeForJson(Map<String, dynamic> data) {
    ...
    data.forEach((key, value) {
      if (value is Timestamp) {
        ...
        cleanData[key] = _sanitizeForJson(value);
      } else {
  ...
  // ─── LEADERBOARD LOGIC (THIS WAS MISSING) ──────────────────────────
